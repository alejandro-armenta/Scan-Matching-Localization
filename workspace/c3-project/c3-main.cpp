...
//NOTE: This is my NDT function that aligns two point clouds.
...
Eigen::Matrix4d NDT(pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt, PointCloudT::Ptr source, Pose startingPose, int iterations){
 	Eigen::Matrix4f sourceTransform = transform3D(startingPose.rotation.yaw, startingPose.rotation.pitch, startingPose.rotation.roll, startingPose.position.x, startingPose.position.y, startingPose.position.z).cast<float>();
  
  	ndt.setMaximumIterations(iterations);
  	ndt.setInputSource(source);
  	pcl::PointCloud<pcl::PointXYZ>::Ptr alignedPC(new pcl::PointCloud<pcl::PointXYZ>);
  
  	ndt.align(*alignedPC, sourceTransform);
...
int main(){
	...
	typename pcl::PointCloud<PointT>::Ptr scanCloud (new pcl::PointCloud<PointT>);
  	...
  	//NOTE: NDT creation outside while loop for performance boost!
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			new_scan = true;
          	...
          	Eigen::Matrix4d transform = NDT(ndt, cloudFiltered, pose, 1);
